"""
This code is where the principle of Don't Repeat Yourself went to die...
"""
import numpy as np
    ...
    for y in range(0,99):
        ...
        if forrest_array[x,y,1] == 1:
            ...
            #continue if the tree is smaller than the current view height
                ...
                continue
#same as above but with the order of the loop reversed
#Y is the column so we're now checking from top down
...
#Exactly the same as the first loop but with the reversed array.
for x in range(0,99):
    current_viewable_height = 0
    for y in range(0,99):
        if forrest_array[x,y,1] == 1:
...
#Exactly the same as the second loop but with the array reversed.
for y in range(0,99):
    current_viewable_height = 0
    for x in range(0,99):
        if forrest_array[x,y,1] == 1:
